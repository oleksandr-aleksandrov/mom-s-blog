...
class Swift_Performance_Setup {
	...
	public function init(){
		$swift_performance_purchase_key = Swift_Performance_Lite::get_option('purchase-key');
		...
		if (isset($_REQUEST['swift-nonce']) && wp_verify_nonce($_REQUEST['swift-nonce'], 'swift-performance-setup') && current_user_can('manage_options')){
			if (isset($_POST['options']) && is_array($_POST['options'])){
				foreach ($_POST['options'] as $key => $value) {
					Swift_Performance_Lite::update_option($key, $value);
				}
			}
			switch ($this->current_step['id']){
				...
				case 'finish':
					...
					Swift_Performance_Lite::update_option('cache-404',1);
	...
	public function step_links() {
		...
		if ($this->current_step['id'] == 'analyze'){
			...
			'<a class="swift-btn swift-btn-green swift-btn-lg swift-use-autoconfig" href="'. esc_url(wp_nonce_url(add_query_arg(array('subpage' => $_GET['subpage'], 'step' => (end($step_keys))), menu_page_url('swift-performance', false)), 'swift-performance-setup', 'swift-nonce')) . '" disabled>'.esc_html__('Use Autoconfigured Settings', 'swift-performance').'</a>'.
			...
			switch ($_REQUEST['ajax-action']){
				...
				case 'cpu':
					...
					if (empty($cpu_benchmark) || $cpu_benchmark >= 10){
						...
						Swift_Performance_Lite::update_option('max-threads', 1);
						$dashicon = 'warning';
						...
						Swift_Performance_Lite::update_option('critical-css', 0);
						Swift_Performance_Lite::update_option('max-threads', 2);
					}
					else {
						Swift_Performance_Lite::update_option('max-threads', 3);
					}
	...
	public static function is_checked($option, $value = '1'){
		if (is_array($option)){
			foreach ($option as $key => $value) {
				if (Swift_Performance_Lite::check_option($key, $value, '!=')){
					return;
				}
			}
			echo ' checked';
		}
		else if (Swift_Performance_Lite::check_option($option, $value)){
			echo ' checked';
		}
	}
}
